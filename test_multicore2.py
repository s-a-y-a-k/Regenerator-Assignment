...
def check_crosstalk(path, core, start_slot, slots_needed):
    for i in range(len(path) - 1):
        ...
        for adj_core in adjacent_cores[core]:
            end_slot = min(start_slot + slots_needed, SLOTS)
            for s in range(start_slot, end_slot):
                if rsa[link][adj_core][s] == 1:
                    return True
    return False
...
def find_best_core(path, slots_needed):
    ...
    for core in range(CORES):
        ...
        for i in range(len(path) - 1):
            u, v = path[i], path[i + 1]
            link = frozenset([u, v])
            total_free += rsa[link][core].count(0)
...
def allocate_path(path, slots_needed):
    ...
    total_slots = slots_needed + 1  # +1 for guard band

    selected_core = find_best_core(path, slots_needed)
    print("Trying to allocate on core", selected_core + 1)
    ...
    for start_slot in range(SLOTS - total_slots + 1):
        ...
        for i in range(len(path) - 1):
            u, v = path[i], path[i + 1]
            link = frozenset([u, v])
            for s in range(start_slot, start_slot + total_slots):
                if rsa[link][selected_core][s] != 0:
                    available = False
                    break
            if not available:
                break
        ...
        if available:
            print("  Allocating on core", selected_core + 1, "slots", start_slot, "-", start_slot + total_slots - 1)      
            for i in range(len(path) - 1):
                u, v = path[i], path[i + 1]
                link = frozenset([u, v])
