...
def find_best_core(path, slots_needed):
    ...
    for core in range(CORES):
        ...
        for start_slot in range(SLOTS - slots_needed):
            ...
            if check_crosstalk(path, core, start_slot, slots_needed):
                print("Crosstalk found! ")
                continue
            ...
            if valid:
                # Count how many free slots are available in this segment
                total_free = 0
                for i in range(len(path) - 1):
                    u, v = path[i], path[i + 1]
                    link = frozenset([u, v])
                ...
                print("Total free are: ", total_free)

                if total_free > max_free_slots:
                    max_free_slots = total_free
                    best_core = core
                    best_start_slot = start_slot
                    print("Max free slot: ", max_free_slots," best core: ",core+1," best start slot: ",best_start_slot)
