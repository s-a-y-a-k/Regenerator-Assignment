...
def dijkstra(source, destination):
    n = len(graph)
    visited = [False] * n
    distance = [99999] * n # Use the same large number as in the graph
    previous = [None] * n
    distance[source] = 0
        ...
        for start_slot in range(SLOTS - (slots_needed + 1) + 1):
            ...
            if valid:
                ...
                for i in range(len(path) - 1):
                    u, v = path[i], path[i + 1]
                    link = frozenset([u, v])
                    segment = rsa[link][core][start_slot:]
...
def request(source, destination):
    ...
    if path:
        ...
        if not valid_mods:
            print(f"No suitable modulation format found for SP length: {sp_length}")
            request_log.append({
                'source': nodes[source], 'destination': nodes[destination], 'path': path,
                'sp_length': sp_length, 'bandwidth': bandwidth, 'modulation': "N/A",
                'slots_needed': "N/A", 'success': False
            })
            return
        ...
        print(f"\nSource: {nodes[source]}")
        print(f"Destination: {nodes[destination]}")
        print(f"Path: {' -> '.join(path)}")
        print(f"SP Length (km): {sp_length}")
        print(f"Bandwidth (Gbps): {bandwidth}")
        print(f"Selected Modulation Format: {best_mod['name']}")
        print(f"Slots needed (excluding guard bit): {slots_needed}")

        success = allocate_path(path, slots_needed)
        
        request_log.append({
            'source': nodes[source], 'destination': nodes[destination], 'path': path,
            'sp_length': sp_length, 'bandwidth': bandwidth, 'modulation': best_mod['name'],
...
for i, req in enumerate(request_log):
    print(f"Request {i+1}: Source={req['source']}, Destination={req['destination']}, Path={' -> '.join(req['path']) if req['path'] else 'N/A'}, SP Length={req['sp_length']} km, Bandwidth={req['bandwidth']} Gbps, Modulation={req['modulation']}, Slots Needed={req['slots_needed']}, Success={req['success']}")
    if not req['success']:
        fail_count += 1
