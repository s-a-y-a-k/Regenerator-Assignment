...
def find_first_fit_for_path(path, slots_needed):
    """
    Finds the first available core and starting slot that can accommodate the request
    across the entire path, enforcing spectrum and core continuity.
    """
    total_slots_required = slots_needed + 1

    # First-Fit: Iterate from the first core and first slot index
    for core in range(CORES):
        for start_slot in range(SLOTS - total_slots_required + 1):
            if causes_strong_crosstalk(core, start_slot, slots_needed, path):
                continue

            # Check for availability across the entire path for this core/slot block
            is_block_available_on_path = True
            for i in range(len(path) - 1):
                u, v = path[i], path[i + 1]
                link = frozenset([u, v])
                
                # Check each slot in the required block on the current link
            ...
            if is_block_available_on_path:
                return core, start_slot # Found the first fit

    return -1, -1 # No suitable block was found


def allocate_path(path, slots_needed):
    """
    Allocates resources for the entire path using the corrected First-Fit strategy.
    """
    # Find the first available core and slot block across the ENTIRE path
    selected_core, selected_start = find_first_fit_for_path(path, slots_needed)
    
    if selected_core == -1:
        return False # Allocation failed

    # If a valid spot is found, allocate it on all links of the path
    for i in range(len(path)-1):
        u, v = path[i], path[i+1]
        link = frozenset([u, v])
