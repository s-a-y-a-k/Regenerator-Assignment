...
# --- Regenerator-Aware Placement (Unchanged) ---
def find_optimal_regenerator_placement(path, bandwidth):
    if len(path) == 2: # Handle direct link case
...
def request(source, destination):
    ...
    primary_segments = find_optimal_regenerator_placement(primary_path, bandwidth)
    backup_segments = find_optimal_regenerator_placement(backup_path, bandwidth)
