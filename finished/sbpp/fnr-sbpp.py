...
def place_fnr_regenerators(path, bandwidth):
    ...
    while current_node_idx < len(path) - 1:
        ...
        end_idx = segment_start_idx # Initialize end_idx
        ...
        for possible_end_idx in range(segment_start_idx + 1, len(path)):
            ...
            # Look ahead to the *next* potential link to see if it forces a modulation step-down
            if possible_end_idx + 1 < len(path):
                ...
                # FNR Condition: End segment if next hop is unreachable or less efficient
                if not next_mod or next_mod['efficiency'] < current_mod['efficiency']:
                    end_idx = possible_end_idx
                    break
            
            # If we reach here, it means we can continue extending
            end_idx = possible_end_idx

        # Create the segment based on the determined end_idx
        ...
        if not final_mod: return None # Should not happen, but a safeguard
...
# --- Main Request Handling Logic (UPDATED to call FNR) ---
