...
def find_first_fit_for_link(link, slots_needed, path_segment):
    """
    Finds the first available core and starting slot that can accommodate the request on a single link.
    """
    ...
    for core in range(CORES):
        for start_slot in range(SLOTS - total_slots_required + 1):
            ...
            # If the block is free, we have found the first fit. Return immediately.
            if is_block_free:
...
def allocate_path(path, slots_needed):
    ...
    for i in range(len(path) - 1):
        ...
        # Use the new First-Fit function
        best_core, best_start_slot = find_first_fit_for_link(link, slots_needed, [u, v])
