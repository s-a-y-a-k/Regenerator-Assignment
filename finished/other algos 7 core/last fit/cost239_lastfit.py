...
def find_best_core(path, slots_needed):
    ...
    prioritizing the HIGHEST slot index first ("Last-Fit" or "Highest Index First").
    """
    ...
    for core in range(CORES):
        ...
        # We use reversed() to search from the highest slot index downwards.
        for start_slot in reversed(range(SLOTS - total_slots_required + 1)):
            ...
            # If the block is available, we have found the highest-indexed fit. Return immediately.
