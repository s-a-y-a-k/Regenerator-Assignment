...
def find_random_fit_for_link(link, slots_needed, path_segment):
    """
    Finds an available core and starting slot using a "Random-Fit" strategy.
    """
    total_slots_required = slots_needed + 1
    
    # Create a list of all possible placements (core, start_slot)
    possible_placements = []
    for core in range(CORES):
        for start_slot in range(SLOTS - total_slots_required + 1):
            possible_placements.append((core, start_slot))
    
    # Shuffle the list to randomize the search order
    random.shuffle(possible_placements)

    # Iterate through the shuffled list of possible placements
    for core, start_slot in possible_placements:
        ...
        # If the block is free, we have found a valid random fit. Return immediately.
        if is_block_free:
...
def allocate_path(path, slots_needed):
    ...
    for i in range(len(path) - 1):
        ...
        # Use the new Random-Fit function
        best_core, best_start_slot = find_random_fit_for_link(link, slots_needed, [u, v])
