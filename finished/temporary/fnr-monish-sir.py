...
# --- NEW: Helper function to prepare request details (MODIFIED for smarter pathfinding) ---
def prepare_request(source, destination):
    ...
    # This ensures pathfinding only considers routes made of viable segments.
    ...
    # 1. Start with the filtered graph again.
    backup_temp_graph = copy.deepcopy(filtered_graph)
    # 2. Remove the links used by the primary path to ensure disjointedness.
    for i in range(len(primary_path) - 1):
        u_idx, v_idx = nodes.index(primary_path[i]), nodes.index(primary_path[i+1])
        backup_temp_graph[u_idx][v_idx] = 99999
        backup_temp_graph[v_idx][u_idx] = 99999
    # 3. Find the shortest path on this doubly-filtered graph.
