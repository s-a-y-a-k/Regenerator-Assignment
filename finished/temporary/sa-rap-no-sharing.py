...
# --- MODIFIED: allocate_segments (Removed all sharing logic) ---
    ...
    for seg_idx, seg in enumerate(segments):
        ...
        for start_slot in range(SLOTS - total_slots_with_guard + 1):
            ...
            for i in range(len(seg['path']) - 1):
                ...
                # NEW: Simplified check. Both primary and backup paths MUST find 'free' slots.
            ...
            if is_block_available:
                # NEW: Simplified allocation. No 'hybrid_backup' status.
                for i in range(len(seg['path']) - 1):
                    link = frozenset([seg['path'][i], seg['path'][i+1]])
        ...
        if not segment_allocated:
            # If this segment failed, roll back any previous segments for THIS request
    ...
    for seg, alloc in zip(segments, allocations):
        ...
        for i in range(len(seg['path']) - 1):
            ...
            for s in range(start, end):
                ...
                # Remove 'G' if it's the guard slot
...
# This phase is still relevant for deciding the *order* of allocation,
# even if the slots themselves aren't shared.
...
for req_id_to_place in pending_backup_req_ids:
    ...
    for set_idx, current_set in enumerate(mutually_exclusive_sets):
        ...
        # This part of the logic groups based on *potential* to share,
        # which is still a valid heuristic for ordering.
        for existing_req_id in current_set:
...
# --- Phase 3: Allocate Backup Paths from Sets (Unchanged logic, but uses new allocate_segments) ---
...
for i, me_set in enumerate(mutually_exclusive_sets):
    ...
    for req_id in me_set:
        ...
        # The new allocate_segments is called here. It will no longer share.
        ...
        if b_allocations:
            ...
            # The new deallocate_path is called here.
...
def calculate_and_print_statistics():
    ...
    if sa_rap_used_count == 0:
        print("   - SA-RAP was not used. For this network, FLR was always optimal.")
    else:
