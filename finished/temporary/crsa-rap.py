...
for i in range(len(graph)):
    for j in range(len(graph)):
        ...
        if p_allocations:
            ...
            # This is where regenerator counts for successful primaries would be updated
            if len(p_segments) > 1:
                for seg in p_segments[:-1]:
                    regenerator_node = seg['path'][-1]
                    regenerator_usage[regenerator_node] += 1
        else:
...
for i, me_set in enumerate(mutually_exclusive_sets):
    ...
    for req_id in me_set:
        ...
        if b_allocations:
            ...
            # Update regenerator counts for successful backup
            if len(conn_info['backup_details']) > 1:
                for seg in conn_info['backup_details'][:-1]:
                    regenerator_node = seg['path'][-1]
                    regenerator_usage[regenerator_node] += 1
            ...
            # Roll back primary path's regenerator usage
            if len(conn_info['primary_details']) > 1:
                for seg in conn_info['primary_details'][:-1]:
                    regenerator_node = seg['path'][-1]
                    regenerator_usage[regenerator_node] -= 1
