...
# --- Main Request Handling Logic (UPDATED with smarter pathfinding) ---
def request(source, destination):
    ...
    # --- MODIFIED: The backup path logic needs to use a similarly filtered graph ---
    # We will modify the get_disjoint_path function to accept the filtered_graph
    # First, let's create a temp graph for the backup path, removing primary links
    backup_temp_graph = copy.deepcopy(filtered_graph) # Start with the already filtered graph
    for i in range(len(primary_path) - 1):
        u_idx, v_idx = nodes.index(primary_path[i]), nodes.index(primary_path[i+1])
        backup_temp_graph[u_idx][v_idx] = 99999
        backup_temp_graph[v_idx][u_idx] = 99999

    # Now find the backup path on this doubly-filtered graph
