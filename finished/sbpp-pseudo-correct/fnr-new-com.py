...
# --- NEW: Helper for Continuity-First Strategy ---
def create_single_segment_plan(path, bandwidth):
    """Creates a single-segment plan for an entire path, if physically possible."""
    if not path or len(path) < 2:
        return None
    
    total_dist = sum(graph[nodes.index(path[i])][nodes.index(path[i+1])] for i in range(len(path)-1))
    mod = get_best_mod(total_dist)
    
    if not mod: # Path is too long for any modulation format
        return None
...
# --- First Narrowest Reach (FNR) Regenerator Placement (Unchanged) ---
    ...
    while current_node_idx < len(path) - 1:
        ...
        for possible_end_idx in range(segment_start_idx + 1, len(path)):
            ...
            if possible_end_idx + 1 < len(path):
                ...
                next_dist = sum(graph[nodes.index(next_sub_path[i])][nodes.index(next_sub_path[i+1])] for i in range(len(next_sub_path)-1))
        ...
        final_segment_path = path[segment_start_idx : end_idx + 1]
        ...
        slots_needed = math.ceil(bandwidth / (final_mod['capacity'] * final_mod['efficiency']))
        segments.append({
...
# --- Main Request Handling Logic (UPDATED WITH TWO-ATTEMPT STRATEGY) ---
def request(source, destination):
    ...
    # --- ATTEMPT 1: Continuity-First (No Regenerators) ---
    print("  Attempt 1: Trying with full continuity...")
    primary_segments_v1 = create_single_segment_plan(primary_path, bandwidth)
    backup_segments_v1 = create_single_segment_plan(backup_path, bandwidth)
    
    if primary_segments_v1 and backup_segments_v1:
        primary_allocations_v1 = allocate_segments(primary_segments_v1, req_id, 'primary')
        if primary_allocations_v1:
            backup_allocations_v1 = allocate_segments(backup_segments_v1, req_id, 'backup', primary_path)
            if backup_allocations_v1:
                print("Request SUCCESS (on Attempt 1: Continuity-First)")
                connection_info = {
                    'primary_path': primary_path, 'primary_details': primary_segments_v1, 'primary_allocations': primary_allocations_v1,
                    'backup_path': backup_path, 'backup_details': backup_segments_v1, 'backup_allocations': backup_allocations_v1,
                    'success': True
                }
                ...
                deallocate_path(primary_segments_v1, primary_allocations_v1, req_id)
    
    # --- ATTEMPT 2: FNR Fallback (If Attempt 1 failed) ---
    print("  Attempt 1 failed. Attempt 2: Falling back to FNR strategy...")
    primary_segments_v2 = place_fnr_regenerators(primary_path, bandwidth)
    backup_segments_v2 = place_fnr_regenerators(backup_path, bandwidth)

    if not primary_segments_v2 or not backup_segments_v2:
        log_entry['reason'] = "Could not place regenerators on a path."
        print(f"Request FAILED: {log_entry['reason']}")
        request_log.append(log_entry)
        return

    primary_allocations_v2 = allocate_segments(primary_segments_v2, req_id, 'primary')
    if not primary_allocations_v2:
        log_entry['reason'] = "No spectrum available for primary path (FNR)."
        print(f"Request FAILED: {log_entry['reason']}")
        request_log.append(log_entry)
        return

    backup_allocations_v2 = allocate_segments(backup_segments_v2, req_id, 'backup', primary_path)
    if not backup_allocations_v2:
        log_entry['reason'] = "No spectrum available for shared backup path (FNR)."
        print(f"Request FAILED: {log_entry['reason']} Rolling back primary allocation.")
        deallocate_path(primary_segments_v2, primary_allocations_v2, req_id)
        request_log.append(log_entry)
        return

    print("Request SUCCESS (on Attempt 2: FNR Fallback)")
    connection_info = {
        'primary_path': primary_path, 'primary_details': primary_segments_v2, 'primary_allocations': primary_allocations_v2,
        'backup_path': backup_path, 'backup_details': backup_segments_v2, 'backup_allocations': backup_allocations_v2,
        'success': True
    }
    active_connections[req_id] = connection_info
    log_entry.update(connection_info)
...
def calculate_and_print_statistics():
    ...
    # f) Total Number of Regenerations (NEWLY ADDED)
    ...
    print(f"g) Total Execution Time: {end_time - start_time:.4f} seconds")
