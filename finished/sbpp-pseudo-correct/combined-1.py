...
def calculate_and_print_statistics(sim_start_time):
    ...
    total_guard_bands = sum(len(req.get('primary_details', [])) + len(req.get('backup_details', [])) for req in request_log if req['success'])
    ...
    print(f"f) Simulation Execution Time: {sim_end_time - sim_start_time:.4f} seconds")
