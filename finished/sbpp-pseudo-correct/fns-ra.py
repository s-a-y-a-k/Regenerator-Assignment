...
# Modulation formats are sorted by spectral efficiency (highest to lowest)
mod_formats = [
...
def get_mod_for_distance(distance):
    """Gets the most efficient mod format for a given distance."""
    for mod in mod_formats:
        if mod['reach'] >= distance:
            return mod
    return None
...
# --- NEW: FNS-RA (First-Fit Network Spectrum and Regenerator Allocation) ---
def place_fns_ra_regenerators(path, bandwidth):
    """
    Implements the FNS-RA regenerator placement and modulation selection strategy.
    """
    ...
    while current_node_idx < len(path) - 1:
        ...
        # --- PLAN A: Try to establish the longest transparent segment with the MOST efficient MF ---
        ...
        for possible_end_idx in range(segment_start_idx + 1, len(path)):
            ...
            if current_dist <= most_efficient_mf['reach']:
                farthest_reach_idx = possible_end_idx # This is a valid segment, keep extending
            else:
                break # Path is too long for the best MF, stop trying to extend
        
        if farthest_reach_idx != -1:
            # PLAN A SUCCESS: A segment with the most efficient MF was found.
            ...
            segments.append({
                ...
                'dist': final_dist,
                'mod': most_efficient_mf['name'],
                'slots': slots_needed
            })
            current_node_idx = farthest_reach_idx
            continue # Restart the loop for the next segment

        # --- PLAN B: If Plan A fails, find the SHORTEST segment with any OTHER MF ---
        else:
            # The shortest possible segment is just the next single link.
            ...
            fallback_mf = None
            # Iterate through the *other* MFs to find the first one that works
            for mf in mod_formats[1:]:
                if shortest_segment_dist <= mf['reach']:
                    fallback_mf = mf
                    break
            
            if fallback_mf:
                # PLAN B SUCCESS: Found a less efficient MF for the short segment.
                slots_needed = math.ceil(bandwidth / (fallback_mf['capacity'] * fallback_mf['efficiency']))
                segments.append({
                    'path': shortest_segment_path,
                    'dist': shortest_segment_dist,
                    'mod': fallback_mf['name'],
                    'slots': slots_needed
                })
                current_node_idx = segment_start_idx + 1
            else:
                # COMPLETE FAILURE: Even the shortest segment is not possible with any MF.
                # This indicates the path is unserviceable.
                return None
        ...
        for start_slot in range(SLOTS - total_slots_with_guard + 1):
            ...
            for i in range(len(seg['path']) - 1):
                ...
                for s in range(start_slot, start_slot + total_slots_with_guard):
                    ...
                    if path_type == 'primary' and slot_info['status'] != 'free':
                        ...
                        break
                    if path_type == 'backup':
                        if slot_info['status'] == 'primary' or slot_info['status'] == 'guard':
                            is_block_available = False
                            break
            ...
            if is_block_available:
                for i in range(len(seg['path']) - 1):
                    ...
                    if path_type == 'primary' or (path_type == 'backup' and rsa[link][guard_slot_index]['status'] == 'free'):
                        rsa[link][guard_slot_index] = {'status': 'guard', 'request_id': request_id}
...
def request(source, destination):
    ...
    # Using FNS-RA for regenerator placement
    primary_segments = place_fns_ra_regenerators(primary_path, bandwidth)
    backup_segments = place_fns_ra_regenerators(backup_path, bandwidth)
    
    if not primary_segments or not backup_segments:
        log_entry['reason'] = "Could not place regenerators on a path (FNS-RA failure)."
        print(f"Request FAILED: {log_entry['reason']}")
        request_log.append(log_entry)
        return
