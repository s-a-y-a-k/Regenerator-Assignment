...
# --- NEW: Logic to decide on regeneration strategy ---
def process_path_for_segments(path, path_dist, bandwidth):
    ...
    - Otherwise, it uses the FNR strategy to place regenerators.
    """
    ...
    if best_mod_for_whole_path:
        ...
        # Path is too long, FNR regenerator placement is required.
        print("  Path is too long for any single modulation format. Using FNR.")
        return place_fnr_regenerators(path, bandwidth)
...
def request(source, destination):
    ...
    if not primary_segments or not backup_segments:
        log_entry['reason'] = "Could not form segments for a path (likely too long)."
        print(f"Request FAILED: {log_entry['reason']}")
        request_log.append(log_entry)
        return
...
def calculate_and_print_statistics():
    ...
    for req in request_log:
        if req['success']:
            # Each segment requires one guard band
