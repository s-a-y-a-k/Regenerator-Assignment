...
# --- Closest-to-Start Regenerator Placement (Unchanged) ---
...
# --- NEW: Allocation Logic with Spectrum Conversion ---
...
def allocate_segments(segments, request_id, path_type, primary_path_of_this_request=None):
    """
    Allocates resources segment by segment, allowing for spectrum conversion.
    Handles both primary and backup path logic.
    """
    ...
    for seg_idx, seg in enumerate(segments):
            ...
            if is_block_available:
                ...
                for i in range(len(seg['path']) - 1):
                    ...
                    for s in range(start_slot, start_slot + slots_needed):
                        # For backup, only overwrite if free
        ...
        if not segment_allocated:
            # If any segment fails, rollback previous segments of THIS request and fail
...
def deallocate_path(segments, allocations, request_id):
    """Frees up resources for a given request, segment by segment."""
    if not segments or not allocations: return
    for seg, alloc in zip(segments, allocations):
        start = alloc['start_slot']
        end = start + alloc['slots_needed'] + 1 # Include guard bit
        for i in range(len(seg['path']) - 1):
            link = frozenset([seg['path'][i], seg['path'][i+1]])
...
def request(source, destination):
    ...
    if not backup_allocations:
        ...
        deallocate_path(primary_segments, primary_allocations, req_id) # Rollback
        request_log.append(log_entry)
        return
...
for i in range(len(graph)):
    for j in range(len(graph)):
        if i != j:
            request(i, j)
            show_slots() # Uncomment to see slot usage after each request

# --- Final Reporting (UPDATED for per-segment allocation details) ---
