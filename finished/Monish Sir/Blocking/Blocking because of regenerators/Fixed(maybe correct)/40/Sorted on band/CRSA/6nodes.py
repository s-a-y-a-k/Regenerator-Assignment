...
SLOTS = 200
# Updated 6-Node Graph for Algorithm Differentiation
# Distances are chosen to cross modulation reach thresholds (250, 500, 1000, 2000)
# 7-Node Graph: Added Node G to create a parallel path A-G-D
graph = [
...
def get_disjoint_path(primary_path, source, destination):
    ...
    for i in range(len(primary_path) - 1):
        ...
        temp_graph[u_idx][v_idx] = 99999
        temp_graph[v_idx][u_idx] = 99999
...
# --- MODIFIED: CRSA-RAP Algorithm to prioritize cheapest nodes globally ---
def run_crsa_rap(source_node, dest_node, bandwidth, flr_segments, backup_temp_graph):
    """
    Implements Cost at Regeneration Site Aware Regenerator Assignment.
    Now considers ALL network nodes as potential regenerator sites to find the 
    absolute cheapest nodes, then builds the path segments between them.
    """
    num_regenerators_flr = len(flr_segments) - 1
    # Potential regenerators are all nodes except source and destination
    potential_regen_nodes = [n for n in nodes if n != source_node and n != dest_node]

    if num_regenerators_flr < 1:
        return None

    print(f"     - Running Global CRSA-RAP with {num_regenerators_flr} regenerator(s) to find cheapest placement...")

    # Find all combinations of nodes from the entire network
    possible_placements = itertools.combinations(potential_regen_nodes, num_regenerators_flr)

    valid_placements = []

    for placement_combo in possible_placements:
        # For a specific set of nodes, we need to find the best path/order.
        # We try all permutations of these chosen regenerators to see if a valid path exists.
        for sorted_regen_nodes in itertools.permutations(placement_combo):
            current_segments_temp = []
            is_valid_placement = True
            
            # Build segments: Source -> R1 -> R2 ... -> Destination
            full_route_sequence = [source_node] + list(sorted_regen_nodes) + [dest_node]
            
            for i in range(len(full_route_sequence) - 1):
                u_name, v_name = full_route_sequence[i], full_route_sequence[i+1]
                u_idx, v_idx = nodes.index(u_name), nodes.index(v_name)
                
                # Find the shortest path for this specific segment
                seg_path, seg_dist = find_shortest_path(u_idx, v_idx, backup_temp_graph)
                
                if not seg_path:
                    is_valid_placement = False
                    break
                
                seg_details = calculate_segment_details(seg_path, bandwidth)
                if not seg_details:
                    is_valid_placement = False
                    break
            ...
            # Calculate metrics for this valid end-to-end path
            current_total_link_slots = calculate_total_link_slots(current_segments_temp)
            ...
            valid_placements.append({
                ...
                'total_link_slots': current_total_link_slots,
    ...
    # Priority: 1. Lowest Usage Cost, 2. Lowest Total Link-Slots
...
def process_path_for_segments(path, path_dist, bandwidth, backup_temp_graph):
    """
    ...
    # Case 3: Try CRSA-RAP
    crsa_rap_best_placement = run_crsa_rap(path[0], path[-1], bandwidth, flr_segments, backup_temp_graph)
...
def prepare_request(source, destination):
    ...
    for i in range(len(primary_path) - 1):
        u_idx, v_idx = nodes.index(primary_path[i]), nodes.index(primary_path[i+1])
        backup_temp_graph[u_idx][v_idx] = 999999
        backup_temp_graph[v_idx][u_idx] = 999999
    ...
    primary_segments = process_path_for_segments(primary_path, primary_dist, bandwidth, filtered_graph)
    if not primary_segments:
        log_entry['reason'] = "Could not place regenerators on primary path."
        print(f"FAILED: {log_entry['reason']}")
        return None, log_entry, None, None, None, None, None, None
    
    backup_segments = process_path_for_segments(backup_path, backup_dist, bandwidth, backup_temp_graph)
