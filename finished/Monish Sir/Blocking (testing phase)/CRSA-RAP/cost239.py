...
max_regenerators_at_a_node = 10 # <<<<<<<<<<<<< You can change this value

request_log = []
request_id_counter = 0
active_connections = {}
regenerator_usage = {node: 0 for node in nodes} # This remains the "tentative cost" dictionary
    ...
    for link, slots in sorted_links:
        ...
        for slot_info in slots:
            if slot_info['status'] == 'free':
                ...
                rid = slot_info['request_id'][0] if slot_info['request_id'] else '?'
                slot_display.append(f"P{rid}")
            elif slot_info['status'] == 'backup':
                rid = slot_info['request_id'][0] if slot_info['request_id'] else '?'
                slot_display.append(f"B{rid}")
...
# --- Logic to decide on regeneration strategy using CRSA-RAP (Unchanged) ---
...
# --- MODIFIED: Helper function to prepare request details (Now includes blocking check) ---
def prepare_request(source, destination):
    ...
    # <<< NEW: Check for regenerator blocking after adding primary
    primary_regen_nodes = get_needed_regenerators(primary_segments)
    for node in primary_regen_nodes:
        if regenerator_usage[node] > max_regenerators_at_a_node:
            log_entry['reason'] = f"Regenerator limit ({max_regenerators_at_a_node}) exceeded at Node '{node}' (Primary Path Planning)."
            print(f"FAILED: {log_entry['reason']}")
            # Roll back the cost we just added
    ...
    # <<< NEW: Check for regenerator blocking after adding backup
    backup_regen_nodes = get_needed_regenerators(backup_segments)
    # We must check all nodes used by *both* paths
    all_regen_nodes = set(primary_regen_nodes + backup_regen_nodes) 
    for node in all_regen_nodes:
        if regenerator_usage[node] > max_regenerators_at_a_node:
            log_entry['reason'] = f"Regenerator limit ({max_regenerators_at_a_node}) exceeded at Node '{node}' (Backup Path Planning)."
            print(f"FAILED: {log_entry['reason']}")
            # Roll back *both* costs we added
            update_regenerator_cost(primary_segments, 'subtract') 
            update_regenerator_cost(backup_segments, 'subtract') 
            return None, log_entry, None, None, None, None
    
    # If we get here, both paths are planned and *within* regenerator limits
    return req_id, log_entry, primary_path, backup_path, primary_segments, backup_segments

# --- (REMOVED) update_final_regenerator_counts is no longer needed ---
# The live 'regenerator_usage' dictionary is the final count.
...
for i in range(len(graph)):
    for j in range(len(graph)):
        ...
        if not req_id:
            request_log.append(log_entry) # Already logged failure (planning or blocking)
            continue
...
# --- Phase 3: Allocate Backup Paths from Sets (Unchanged) ---
# (No blocking check needed here, it already happened in prepare_request)
...
for i, me_set in enumerate(mutually_exclusive_sets):
    ...
    for req_id in me_set:
        ...
        if b_allocations:
            ...
            # The tentative costs are now final, so we do nothing.
...
def calculate_and_print_statistics():
    ...
    # j) Total Requested Bandwidth
    total_requested_bandwidth = sum(req['bandwidth'] for req in request_log)
    print(f"j) Total Requested Bandwidth: {total_requested_bandwidth} Gbps")

    # k) Total Blocked Bandwidth
    total_blocked_bandwidth = sum(req['bandwidth'] for req in request_log if not req['success'])
    print(f"k) Total Blocked Bandwidth: {total_blocked_bandwidth} Gbps")
    
    # l) Total Successful Regenerated Bandwidth
    ...
    print(f"l) Total Successful Regenerated Bandwidth: {total_successful_regenerated_bandwidth} Gbps")

    # m) Bandwidth Blocking Probability (BBP)
    bbp = (total_blocked_bandwidth / total_requested_bandwidth) if total_requested_bandwidth > 0 else 0
    print(f"m) Bandwidth Blocking Probability (BBP): {bbp:.2%} (Blocked BW / Total Requested BW)")

    # n) Bandwidth Blocking Ratio (BBR) - User's Formula
    ...
    if total_successful_regenerated_bandwidth > 0:
        ...
        print(f"n) Bandwidth Blocking Ratio (BBR): {bbr_user_formula:.2f} (Blocked BW / Successful Regenerated BW)")
    else:
        print("n) Bandwidth Blocking Ratio (BBR): N/A (No successful regenerated bandwidth)")
