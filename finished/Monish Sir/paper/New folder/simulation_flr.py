...
def run_flr_simulation(slots_limit, regen_limit):
    # --- NEW: Set the global variable inside the function ---
    global max_regenerators_at_a_node, SLOTS
    max_regenerators_at_a_node = regen_limit
    SLOTS = slots_limit # Set SLOTS from the argument
    
    # --- NEW: Reset all global states ---
    # You MUST reset these so each simulation run is fresh
    global rsa, request_log, request_id_counter, active_connections, regenerator_usage
    global total_slots_requested_all_links, total_slots_failed_all_links, start_time
    ...
    # ... (Paste ALL your code from Phase 1 to the end of Phase 4) ...
    ...
    # ... (Your get_simulation_results() function) ...

    def get_simulation_results(rsa, request_log, request_id_counter, active_connections, regenerator_usage, 
        edges, SLOTS, start_time, total_slots_failed_all_links, total_slots_requested_all_links):
        ...
        results = {
            ...
            'bandwidth_blocking_ratio': bbr,
            'total_regenerations': total_regenerations,
            'std_dev_regenerator_usage': std_dev_usage,
            'network_fragmentation': average_fragmentation,
            'highest_slot_index': highest_slot_index_found,
            'total_utilized_slots': total_utilized_slots,
            'total_guard_bands': total_guard_bands,
            'execution_time': execution_time
        }
        return results
    
    return get_simulation_results(rsa, 
                         request_log, # <--- ADDED request_log
                         request_id_counter, 
                         active_connections, 
                         regenerator_usage,  # <--- SWAPPED ORDER TO MATCH
                         edges,              # <--- SWAPPED ORDER TO MATCH
                         SLOTS,              # <--- SWAPPED ORDER TO MATCH
                         start_time,         # <--- SWAPPED ORDER TO MATCH
                         total_slots_failed_all_links, 
                         total_slots_requested_all_links)

if __name__ == "__main__":
    results = run_flr_simulation(slots_limit=250, regen_limit=3)
    print("--- FLR RESULTS (Single Run) ---")
    print(results)