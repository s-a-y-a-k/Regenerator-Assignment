...
def get_best_mod_for_distance(distance):
    """Gets the most efficient mod format for a given distance."""
    # Since mod_formats is sorted by efficiency, the first match is the best.
...
# --- MODIFIED: High-level segmentation planner ---
def plan_path_segments(path, bandwidth):
    """
    First, checks if the entire path can be handled by a single modulation format.
    If not, it calls the FNS-RA regenerator placement strategy.
    """
    # Calculate the total distance of the entire path
    ...
    best_mod = get_best_mod_for_distance(total_dist)
    
    if best_mod:
        # Direct connection is possible, no regenerators needed. Create one segment.
        print(f"  Path can be established in a single segment with {best_mod['name']}.")
        ...
        # Path is too long, must use FNS-RA to place regenerators.
        print("  Path is too long for any single modulation format. Using FNS-RA.")
...
def request(source, destination):
    ...
    # Use the new high-level planner for segmentation
    print("FOR PRIMARY PATH: ")
    primary_segments = plan_path_segments(primary_path, bandwidth)
    print("FOR BACKUP PATH: ")
    backup_segments = plan_path_segments(backup_path, bandwidth)
    
    if not primary_segments or not backup_segments:
        log_entry['reason'] = "Could not create segments for a path."
        print(f"Request FAILED: {log_entry['reason']}")
        request_log.append(log_entry)
        return
