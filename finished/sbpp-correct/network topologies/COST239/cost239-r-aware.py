...
# --- Regenerator-Aware (R-Aware) Placement (MODIFIED) ---
def place_regenerator_aware(path, bandwidth):
    """
    Implements the R-Aware strategy for paths requiring regeneration.
    Finds the optimal single regenerator placement to minimize spectrum usage.
    """
    if not path or len(path) < 2:
        return None

    # Find the optimal single regenerator placement
...
def process_path_for_segments(path, path_dist, bandwidth):
    ...
    - Otherwise, it uses the R-Aware strategy to place a regenerator.
    """
    ...
    if best_mod_for_whole_path:
        ...
        # Path is too long, R-Aware regenerator placement is required.
        print("  Path is too long for any single modulation format. Using R-AWARE.")
        return place_regenerator_aware(path, bandwidth)
