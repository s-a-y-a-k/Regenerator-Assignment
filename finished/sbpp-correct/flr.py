...
def place_flr_regenerators(path, bandwidth):
    ...
    best_mod_for_path = get_best_mod(path_dist)
    if best_mod_for_path:
        slots = math.ceil(bandwidth / (best_mod_for_path['capacity'] * best_mod_for_path['efficiency']))
        return [{'path': path, 'dist': path_dist, 'mod': best_mod_for_path['name'], 'slots': slots}]
