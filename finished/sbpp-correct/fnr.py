...
def place_fnr_regenerators(path, bandwidth):
    ...
    while current_node_idx < len(path) - 1:
        ...
        # Extend the segment one link at a time
        for possible_end_idx in range(segment_start_idx + 1, len(path)):
            ...
            # If adding this link makes the segment invalid, the segment must end at the previous node.
            if not current_mod:
                end_idx = possible_end_idx - 1
                break

            # Look ahead to the *next* potential link
            if possible_end_idx + 1 < len(path):
                ...
                # FNR Condition: End segment if next hop is unreachable or forces a less efficient modulation
                if not next_mod or next_mod['efficiency'] < current_mod['efficiency']:
                    end_idx = possible_end_idx
                    break
            
            # If we reach here, we can continue extending
            end_idx = possible_end_idx

        # Create the final segment based on the determined end_idx
...
# --- Allocation Logic with Spectrum Conversion (MODIFIED) ---
...
def request(source, destination):
    ...
    # Using FNR for regenerator placement
...
def calculate_and_print_statistics():
    ...
    # f) Total Number of Unique Regenerator Nodes
    unique_regenerator_nodes = set()
    for req in request_log:
        if req['success']:
            # A regenerator is placed at the end node of each segment, except for the last one.
            primary_segments = req.get('primary_details', [])
            if len(primary_segments) > 1:
                for i in range(len(primary_segments) - 1):
                    # The last node of the segment's path is the regenerator node.
                    regenerator_node = primary_segments[i]['path'][-1]
                    unique_regenerator_nodes.add(regenerator_node)
    ...
    total_unique_regenerators = len(unique_regenerator_nodes)
    print(f"f) Total Number of Unique Regenerator Nodes: {total_unique_regenerators} and they are: {unique_regenerator_nodes}")
