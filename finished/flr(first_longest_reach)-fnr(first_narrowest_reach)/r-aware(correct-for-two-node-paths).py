...
# --- Regenerator-Aware (Optimal) Placement (Unchanged) ---
...
def process_path(path, path_len, bandwidth):
    ...
    # Handle the case of a direct link with no intermediate nodes
    if len(path) == 2:
        print("  Path is a direct link. Checking for direct connection.")
        ...
        if direct_mod:
            ...
            # This case should be rare, implies a link longer than any modulation reach
            return None

    # If the path has intermediate nodes, find the optimal single regenerator placement.
    print("  Path has intermediate nodes. Attempting Regenerator-Aware placement...")
