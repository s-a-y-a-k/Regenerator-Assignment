...
def place_flr_regenerators(path, bandwidth):
    ...
    Returns a list of segments, each with its own path, mod, and slots.
    """
    if not path:
        return None
    ...
    while current_node_idx < len(path) - 1:
        ...
        for next_node_idx in range(current_node_idx + 1, len(path)):
            ...
            if dist <= MAX_REACH and sub_path[-1] in regenerators:
...
# --- Main Request Handling Logic (UPDATED for detailed logging) ---
...
def request(source, destination):
    ...
    if not primary_segments:
        print("Request FAILED: Could not place regenerators on primary path.")
        request_log.append({'source': nodes[source], 'destination': nodes[destination], 'bandwidth': bandwidth, 'success': False, 'reason': 'FLR failed on primary'})
        return
        
    backup_segments = place_flr_regenerators(backup_path, bandwidth)
    if not backup_segments:
        print("Request FAILED: Could not place regenerators on backup path.")
        request_log.append({'source': nodes[source], 'destination': nodes[destination], 'bandwidth': bandwidth, 'success': False, 'reason': 'FLR failed on backup'})
        return
        
    print("Primary Segments (FLR):")
    for seg in primary_segments:
        print(f"  - Path: {' -> '.join(seg['path'])}, Mod: {seg['mod']}, Slots: {seg['slots']}")
        
    print("Backup Segments (FLR):")
    ...
    # **MODIFICATION: Store detailed segment info in the log**
    log_entry = {
        'source': nodes[source],
